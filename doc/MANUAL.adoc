...
URL format: `+lmdb:FILE+`

This backend stores data in a LMDB memory-mapped database file.
...
* `+lmdb:/var/cache/ccache/ccache.mdb+`

Optional attributes:

* *mapsize*: The size of virtual memory to map. The default is 50 GiB.
* *update-mtime*: If *true*, update the modification time (mtime) of cache
  entries that are read. The default is *false*.
