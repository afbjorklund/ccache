...
[[config_stats_log]] *stats_log* (*CCACHE_STATSLOG*)::
    ...
    If set to a file path, ccache will write results to the specified file.
    This is useful for getting statistics for individual builds.
+
If set to *statsd*, ccache will log using `statsd` instead of to a file.
+
